...
def fortune(bot, update):
    ...
    if(fortunes is None):
        ...
        bot.send_message(chat_id=update.message.chat_id, reply_to_message_id=msg_ID, text="Sorry I'm all out of fortunes, come back later.")
    else:
        fortune = fortunes.values()
        fortune = list(fortune)
        bot.send_message(chat_id=update.message.chat_id, reply_to_message_id=msg_ID, text=fortune[fortune_num])
        fortune_num += random.randint(1, 2)
        if(fortune_num >= len(fortune) - 1):
            fortune_num = random.randint(1, 4)
        #bot.send_message(chat_id=update.message.chat_id, reply_to_message_id=msg_ID, text="New fortune num: {}".format(fortune_num))
